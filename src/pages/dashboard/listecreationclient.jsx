...
export function Listclient() {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:3000/getallClientData/getallClientData", {
  ...
  const handleDownload = (clientId) => {
    ...
    axios.get(`http://localhost:3000/getClientDataByID/getClientDataByID/${clientId}`, {
      headers: { usr: loggedInUSR },
    })
    .then((response) => {
  ...
  const downloadOrderPDF = async (client) => {
    ...
    try {
      const response = await fetch("http://localhost:3000/downloadOrder/downloadOrder", {
        method: "POST",
        headers: {
  ...
  const handleDeleteClient = async (clientId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/deleteClientData/deleteClientData/${clientId}`, {
        method: "DELETE",
      });
