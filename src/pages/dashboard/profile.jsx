...
export function Profile() {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      try {
        console.log(`Fetching user details from: http://localhost:3000/user/getUserDetails/${USR}`);
        const response = await axios.get(`http://localhost:3000/user/getUserDetails/${USR}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3000/updateuser/updateUser/${USR}`, updatedUser);
